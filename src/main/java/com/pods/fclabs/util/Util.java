...
public class Util {
    ...
    public Util(UsuarioRepository usuarioRepository, EnderecoRepository enderecoRepository) {
    ...
    /**
     * @param date
     * @param locale Metodo que devolve um SimpleDateFormat com uma máscara conforme validacao
    ...
    public static MsgRetorno criaMsgRetorno(Integer status, String error, String message, String exceptionClass, String path) {
    ...
    public static MsgRetorno criaMsgRetornoComUsuario(Integer status, String error, String message, String exceptionClass, Usuario usuario, String path) {
        return MsgRetorno.Builder.create()
                .timestamp(new Date(System.currentTimeMillis()))
                .status(status)
                .error(error)
    ...
    public Endereco converteEndereco(EnderecoForm endereco, Usuario usuario) {
        ...
        novoEndereco.setDtCriacao(Util.formatarData(new Date()));
        novoEndereco.setDtUltAlteracao(Util.formatarData(new Date()));
        return novoEndereco;
    }

    public Endereco atualizaEndereco(EnderecoForm endereco, Endereco enderecoAtualizado, Usuario usuario) {
        ...
        enderecoAtualizado.setDtUltAlteracao(Util.formatarData(new Date()));
        return enderecoAtualizado;
    }

    public Usuario encontraUsuario(UUID id){
        Optional<Usuario> usuario = usuarioRepository.findById(id);
        if (!usuario.isPresent()) throw new UsuarioInexistenteException("Usuário não encontrado");
        return usuario.get();
    }

    public Endereco encontraEndereco(UUID id){
        Optional<Endereco> endereco = enderecoRepository.findById(id);
        if (!endereco.isPresent()) throw new EnderecoInexistenteException("Endereco não encontrado");
        return endereco.get();
    }
